...
// le select pour le format de la question
const selectedOption = ref('');

// le select pour le format de la question
const choixMultiple = ref('');

// nombre de proposition
let nombre_proposition = ref<number>(1);
// ajouter proposition
const ajouter_prop = () =>{
   nombre_proposition.value++;
}

// reduire propostion
const reduire_prop = () =>{
   if (nombre_proposition.value > 1) {
    nombre_proposition.value--;
   }
}
...
const appel_offre = () =>{
  ...
  const tableau  = ref([
  {Type_test: 'test psychotechnique',Type_question: 'Choix multiple', enonce_question: 'choisissez la bonne response', question: 'Quels sont les quatres points cardinaux ?', bonne_reponse: "A-NORD" ,lien_image: "", lien_son:"", nombre_point: 5 , niveau_difficulte: 'Facile',Propositions_reponse: [
    "A-NORD", "B-SUD", "C-EST", "D-OUEST"]},
    {Type_test: 'test psychotechnique',Type_question: 'Choix multiple', enonce_question: 'choisissez la bonne response', bonne_reponse: "A-NORD",lien_image: "", lien_son:"", nombre_point: 5 , niveau_difficulte: 'Facile',Propositions_reponse: [
    "A-NORD", "B-SUD", "C-EST", "D-OUEST"]},
    {Type_test: 'test psychotechnique',Type_question: 'Choix multiple', enonce_question: 'choisissez la bonne response', question: 'Quels sont les quatres points cardinaux ?', bonne_reponse: "A-NORD" ,lien_image: "", lien_son:"", nombre_point: 5 , niveau_difficulte: 'Facile',Propositions_reponse: [
    "A-NORD", "B-SUD", "C-EST", "D-OUEST"]},
  ])
...
watch(recherche, nouvelle_valeur => {
  if (nouvelle_valeur !== undefined) { // Vérification si nouvelle_valeur est définie
    tableau.value = originalTableau.value.filter((utilisateur) => {
      // { type_question: string; enonce_question: string; nombre_point: { toString: () => string|string[]; }; lien_image: string;lien_son: string;question: string; niveau_difficulte: string; Proposition_reponse[]: string  }
      return (
        utilisateur.Type_question.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
        utilisateur.niveau_difficulte.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
        utilisateur.nombre_point.toString().includes(nouvelle_valeur.toLowerCase()) // Convertir en chaîne pour la comparaison
      );
    });
  }
});
    ...
    <section class="formulaires" v-if="formulaire">
      ...
      <form class="formulaire" @submit.prevent="handleSubmit">
          <div class="slides">
              <label>               
                      <select required >
                        <option value="question" disabled>Type de tests</option>
                        <option value="test logique">Test logique</option>
                        <option value="test litterature">Test Litterature</option>
                      </select>
                </label>
            <label>
            ...
            <div class="flex">
                <label>               
                      <select required v-model="selectedOption">
                        <option value="question" disabled> Choississez le format de la question</option>
                ...
                <label v-if="selectedOption === 'question'">
                      <input required placeholder=""  type="text" class="input">
                      <span>question</span>
                </label>
                <label v-else-if="selectedOption === 'image'">
            ...
            <div class="flex1"> 
                <label>               
                    <select required v-model="choixMultiple">
                      <option value="" disabled> Choississez le type de question</option>
                      <option value="choix multiple">choix multiple</option>
                      <option value="choix unique">choix unique</option>
                    </select>
                </label>   
                <div class="plus_moins">                 
                  <button v-if="choixMultiple === 'choix multiple'" @click="reduire_prop">-</button>
                  <button v-if="choixMultiple === 'choix multiple'" @click="ajouter_prop">+</button>
                </div>           
                <div class="proposition">                  
                    <label class="label_proposition" v-for="(element, index) in nombre_proposition" :key="index">                  
                      <input required placeholder="" type="numero" class="input">
                        <span>Proposition de reponse</span>
                    </label>
                </div>
                <label>
                      <input required placeholder=""  type="text" class="input">
        ...
        <table>
          <thead>
              <tr>  
                  ...
                  <th>Enoncé de la question</th>
                  <th>Question</th> 
                  <th>Image</th>
                  <th>son</th>
                  <th>Niveau de diffulcuté</th>
          ...
          <tbody>
              <tr v-for="(ligne, index) in tableau" :key="index">
                  <td>{{ ligne.Type_test }}</td>                                             
                  <td class="">{{ligne.Type_question}}</td>   
                  <td class="">{{ligne.enonce_question}}</td>
                  <td class="">{{ligne.question}}</td> 
                  <td class=""><img src="../../../assets/images_yeo/qcm1.png" alt=""></td>
                  <td class="">
                    <audio controls>
                      <source src="../../../assets/audio_yeo/son1.mp3" type="audio/ogg">                      
                      <p>Votre navigateur ne prend pas en charge l'audio HTML5. Voici un <a href="myAudio.mp3">lien vers le fichier audio</a> à la place.</p>
                    </audio>
                  </td> 
                  <td class="">{{ligne.niveau_difficulte}}</td>  
                  <td class="">
                    <ul v-for="(proposition, index) in ligne.Propositions_reponse" :key="index">
                      <li>{{ proposition }}</li>
                    </ul>
                  </td>  
                  <td>{{ ligne.bonne_reponse }}</td>                                     
                  <td>
                      <div class="boutons">
